...
clientsRouter.put("/:id", async (req, res, next) => {
  try {
    ...
    const before = await pool.query(`SELECT * FROM clients WHERE id = $1`, [req.params.id]);
    ...
    const changes = diffFields(before.rows[0], result.rows[0], [
      { key: "name", label: "Name" },
    ...
    await recordAudit(pool, "client", req.params.id, "update", actor, changes);
    res.json(result.rows[0]);
  } catch (err) {
    next(err);
  }
